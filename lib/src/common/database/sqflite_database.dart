...
abstract class SqfliteDatabase {
  ...
  static Future _onCreate(Database db, int version) async {
    await db.execute('''
          CREATE TABLE $_favTable (
              ${Photo.idDbColumn} INTEGER NOT NULL,
  ...
  static Future<String> getLanguageKey() async {
    try {
      ...
      if (records.isNotEmpty) {
        return records.first[_langKey] as String;
      }
    } on Exception catch (e) {
      debugPrint(e.toString());
    }
    ...
    try {
      ...
      if (records.isNotEmpty) {
        final themeMode = ThemeMode.values.firstWhere(
          (e) => e.name == records.first[_themeKey] as String,
  ...
  static Future<int> addFavPhoto(Photo photo) async {
    return await _db.insert(_favTable, photo.toJson());
  }

  static Future<List<Map<String, dynamic>>> queryAllRows() async {
    return await _db.query(_favTable);
  }
